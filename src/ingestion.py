...
def ingest_and_timestamp_data(synthetic_path: str, clinical_path: str, output_dir: str):
    ...
    df_synth['event_timestamp'] = [base_date_synth + timedelta(days=int(d)) for d in random_days]
    ...
    df_clin['event_timestamp'] = [base_date_clin + timedelta(days=int(d)) for d in random_days_clin]
    ...
    df_synth['created_timestamp'] = pd.to_datetime(df_synth['created_timestamp'], utc=True)
    df_clin['event_timestamp'] = pd.to_datetime(df_clin['event_timestamp'], utc=True)
    df_clin['created_timestamp'] = pd.to_datetime(df_clin['created_timestamp'], utc=True)
