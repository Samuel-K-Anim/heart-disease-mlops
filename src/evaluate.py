...
def optimize_threshold():
    ...
    X_val = df.drop(columns=['target', 'patient_id', 'event_timestamp', 'created_timestamp'])
    ...
    print(f"Default 0.50 Threshold Cost:  {costs[np.abs(thresholds - 0.50).argmin()]:.2f}")
    print(f"Optimal t* Threshold Cost:    {min_cost:.2f} (at t* = {optimal_t:.3f})")
    print(f"Cost Savings vs Default:      {costs[np.abs(thresholds - 0.50).argmin()] - min_cost:.2f}")
    ...
    with mlflow.start_run(run_name="threshold_optimization"):
        ...
        plt.plot(thresholds, costs, label='Total Clinical Cost', color='red', linewidth=2)
        plt.axvline(x=optimal_t, color='blue', linestyle='--', label=f'Optimal t* = {optimal_t:.3f}')
