...
def fetch_synthetic_data(store: FeatureStore) -> pd.DataFrame:
    ...
    try:
         ...
         raise FileNotFoundError("Run src/ingestion.py first to generate the parquet files.")
...
def train_base_model():
    ...
    target_col = 'target' 
    features = [col for col in df.columns if col not in ['target', 'patient_id', 'event_timestamp', 'created_timestamp']]
    ...
    xgb_params.update({"objective": "binary:logistic", "eval_metric": "aucpr", "tree_method": "hist", "device": "cuda" if xgb.core.core.XGBoostError else "cpu"})

    # 2. LightGBM Params (Robust defaults for synthetic data)
    lgb_params = {"objective": "binary", "metric": "average_precision", "learning_rate": 0.05, "max_depth": 6, "verbose": -1}

    # 3. CatBoost Params
    cb_params = {"iterations": 300, "depth": 6, "learning_rate": 0.05, "loss_function": "Logloss", "verbose": 0}
    ...
    with mlflow.start_run(run_name="synthetic_ensemble_pretrain"):
        ...
        mlflow.xgboost.log_model(xgb_base, artifact_path="base_xgb", registered_model_name="heart_disease_xgb_base")
        
        # LightGBM
        dtrain_lgb = lgb.Dataset(X, label=y)
        lgb_base = lgb.train(lgb_params, dtrain_lgb, num_boost_round=300)
        mlflow.lightgbm.log_model(lgb_base, artifact_path="base_lgb", registered_model_name="heart_disease_lgb_base")
        
        # CatBoost
        cb_base = cb.CatBoostClassifier(**cb_params)
        cb_base.fit(X, y)
        mlflow.catboost.log_model(cb_base, artifact_path="base_cb", registered_model_name="heart_disease_cb_base")
        
        print(f"Base ensemble registered in MLflow under run ID: {mlflow.active_run().info.run_id}")
